...
def update_topic_stability_data(event_type: str, payload: dict):
    ...
    to the ConsultationPhaseData model and related tables (TopicKeyword, TopicScopeElement).
    ...
    try:
        # Fetch the ConsultationPhaseData instance linked to the session
        # NOTE: This requires fetching ResearchProject first to get the PK link
        consultation_data = ConsultationPhaseData.objects.select_related(
            'project'
        ).get(
            project_id=project_id
        )
    except ConsultationPhaseData.DoesNotExist:
        logger.error("Task %s: ConsultationPhaseData not found for session %s. Aborting.", task_id, project_id)
        return
    ...
    if not new_stability_score or new_stability_score is None:
        new_stability_score = consultation_data.stability_score
    ...
    if not current_research_question or current_research_question is None:
        current_research_question = consultation_data.final_research_question
    ...
    if not updated_summary or updated_summary is None:
        updated_summary = consultation_data.conversation_summary
    ...
    if not last_chat_sequence_number or last_chat_sequence_number is None:
        last_chat_sequence_number = consultation_data.last_analysis_sequence_number

    consultation_data.stability_score = new_stability_score
    consultation_data.final_research_question = current_research_question
    ...
    consultation_data.feasibility_status = determine_feasibility_status(consultation_data.stability_score, is_niche)
    consultation_data.conversation_summary = updated_summary
    consultation_data.last_analysis_sequence_number = last_chat_sequence_number

    # consultation_data.agent_evaluation_count += 1 # Increment evaluation count
    consultation_data.save(update_fields=[
        'stability_score',
    ...
    keywords = consultation_data.project.keywords.all()
    keyword_set = set([keyword.label for keyword in keywords])
    TopicKeyword = consultation_data.project.keywords.model
    new_keywords = []
    ...
    if new_keywords:
        consultation_data.project.keywords.add(*new_keywords, bulk=False)

    scope_elements = consultation_data.project.scope_elements.all()
    scope_element_set = set([(scope_element.label, scope_element.rationale) for scope_element in scope_elements])
    TopicScopeElement = consultation_data.project.scope_elements.model
    new_scope_elements = []
    ...
    if new_scope_elements:
        consultation_data.project.scope_elements.add(*new_scope_elements, bulk=False)

    refined_topic = SimpleNamespace(
            stability_score=consultation_data.stability_score,
            feasibility_status=consultation_data.feasibility_status,
            final_research_question=consultation_data.final_research_question,
            keywords=consultation_data.project.keywords.all(),
            scope_elements=consultation_data.project.scope_elements.all(),
            resource_suggestion=get_resource_suggestion(consultation_data.feasibility_status)
    )
    ...
    send_ws_notification(
        user_id=user_id,
        event_type=CONSULTATION_REFINED_TOPIC,
